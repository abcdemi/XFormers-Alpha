# === Kaggle-Style Trading Strategy Simulation (v11: Corrected and Hardened) ===
# This version fixes the duplicate feature name error and includes:
...
# --- START OF FIX ---
# Step 3.1: Sanitize column names
sanitized_columns = [re.sub(r'[^a-zA-Z0-9_]', '_', col) for col in X.columns]
X.columns = sanitized_columns

# Step 3.2: Programmatically de-duplicate column names
new_cols = []
col_counts = {}
...
if X.columns.duplicated().any():
    print("Warning: Duplicate columns still exist after sanitization.")
else:
    print("Column names sanitized and de-duplicated successfully.")
# --- END OF FIX ---
...
def run_backtest_with_filter(predictions, test_data_indices, spy_full_data, spy_sma):
    ...
    for i in range(len(daily_returns)):
        ...
        if spy_full_data['Close'].loc[current_date] < spy_sma.loc[current_date]:
            ...
            signal = 1 if aligned_preds[i] > 0.5 else 0
...
for fold, (train_idx, test_idx) in enumerate(tscv.split(X)):
    ...
    model_contrarian = LogisticRegression(solver='liblinear', random_state=42); model_contrarian.fit(X_train_scaled, y_train)
    
    params_simple = {'objective': 'binary', 'n_estimators': 50, 'max_depth': 3, 'seed': 42, 'n_jobs': -1, 'verbose': -1}
    model_simple = lgb.LGBMClassifier(**params_simple); model_simple.fit(X_train, y_train)
...
last_fold_test_indices = all_equity_curves[-1].index if all_equity_curves and not all_equity_curves[-1].empty else df.index[-1:]
